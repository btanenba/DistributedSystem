...
public class ClientMasterThreadResponse1 extends Thread {
	...
	public ClientMasterThreadResponse1(Socket clientSocket, Shared share) {
	...
	public void run() {
		System.out.println("Created Connection and Entering ClientMasterThreadResponse1");
		...
		if (!share.finished.isEmpty()) {
			synchronized (share.finished) {
				for (int i = 0; i < share.finished.size(); i++) {
					if (share.finished.get(i).getID() == 1) {
